...
namespace Cloud7CMS.Areas.Identity.Pages.Account
{
    ...
    public class ExternalLoginModel : PageModel
    {
        ...
        private Cloud7CMSUser CreateUser()
        {
            try
            {
                return Activator.CreateInstance<Cloud7CMSUser>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(Cloud7CMSUser)}'. " +
                    $"Ensure that '{nameof(Cloud7CMSUser)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the external login page in /Areas/Identity/Pages/Account/ExternalLogin.cshtml");
            }
        }

        private IUserEmailStore<Cloud7CMSUser> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<Cloud7CMSUser>)_userStore;
        }
    }
}
